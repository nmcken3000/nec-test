...
User data is stored in memory as a plain array in `lib/mockUsers.ts`. There is no persistence — the list resets on page refresh. A real implementation would use a database or API, but for the purposes of this test it keeps the focus on the front-end behaviour.
...
In the App Router, Server Components can be `async`, which means you can fetch data directly in the component without `useEffect` or `getServerSideProps`. If this app were hitting a real API it would look like this:
